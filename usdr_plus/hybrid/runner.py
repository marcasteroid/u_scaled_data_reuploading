"""Runtime helpers for modular hybrid pipeline."""
...
from usdr_plus.hybrid.analysis.summary import summarize_results
from usdr_plus.hybrid.data.datasets import preprocess_and_save_hybrid_datasets
from usdr_plus.hybrid.training.pipeline import run_hybrid_experiments
...
def run_full_pipeline() -> None:
    ...
    with open(log_path, "w", encoding="utf-8") as log_fh, tqdm(
        ...
        def _run_update(done: int, total: int, _n: int, _seed: int) -> None:
            _set_progress(pbar, 10 + int(84 * done / max(total, 1)))
        try:
            with redirect_stdout(log_fh), redirect_stderr(log_fh):
                preprocess_and_save_hybrid_datasets(
            ...
            print(f"Hybrid pipeline complete. Log saved to: {log_path}")
        except Exception as exc:
            _set_progress(pbar, 100)
            raise RuntimeError(f"Hybrid pipeline failed: {exc}. Check log: {log_path}") from exc
        finally:
            stop_spin.set()
